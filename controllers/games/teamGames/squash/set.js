const Set = require("../../../../models/games/teamGames/squash/Set");
const { default: mongoose } = require("mongoose");
const { addSetToSquashGame, removeSetFromSquashGame, updateTeamScores } = require("./squash");
...
exports.addSet = async (req, res) => {
    ...
    try{
        ...
        const addedSetToSquashGame = await addSetToSquashGame(req, res);

        res.json({
            savedSet,
            addedSetToSquashGame
        })
    }
    catch(err) {
        res.status(500).json({
...
exports.removeSet = async (req, res) => {
    ...
    try {
        ...
        const squashGameAfterDeletingSet = await removeSetFromSquashGame(req, res);
             
        res.json({
            deletedSet,
            squashGameAfterDeletingSet
        });
    }
    catch (err) {
        res.status(500).json({
...
exports.setWinnerOfSet = async  (req, res) => {
    ...
    try{
        ...
        if(teamASetScore > teamBSetScore){
            ...
            const squashGameAfterUpdatedScores= await updateTeamScores(req, res);


            res.json({afterSettingWinnerOfSet, squashGameAfterUpdatedScores})
        }
