...
exports.getBadmintonGameById = async (req, res) => {
    try {
        let result = await BadmintonGame.findOne({_id: req.params.eventId});
        res.json(result);
    }
    catch (err) {
        res.status(500).json({
...
exports.deleteBadmintonGame = async (req, res) => {
    try{
        let result = await BadmintonGame.findOneAndDelete({_id: req.params.eventId});
        res.json(result);
    }catch (err) {
        res.status(500).json({
            message: "mongodb error: " + err.message
