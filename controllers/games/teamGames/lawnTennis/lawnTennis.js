...
exports.getLawnTennisGameById = async (req, res) => {
    try {
        let result = await LawnTennisGame.findOne({_id: req.params.eventId});
        res.json(result);
    }
    catch (err) {
        res.status(500).json({
...
exports.deleteLawnTennisGame = async (req, res) => {
    try{
        let result = await LawnTennisGame.findOneAndDelete({_id: req.params.eventId});
        res.json(result);
    }catch (err) {
        res.status(500).json({
            message: "mongodb error: " + err.message
