const Set = require("../../../../models/games/teamGames/lawnTennis/Set");
const { default: mongoose } = require("mongoose");
const { addSetToLawnTennisGame, removeSetFromLawnTennisGame, updateTeamScores } = require("./lawnTennis");
...
exports.addSet = async (req, res) => {
    ...
    try{
        ...
        const addedSetToLawnTennisGame = await addSetToLawnTennisGame(req, res);

        res.json({
            savedSet,
            addedSetToLawnTennisGame
        })
    }
    catch(err) {
        res.status(500).json({
...
exports.removeSet = async (req, res) => {
    ...
    try {
        ...
        const lawnTennisGameAfterDeletingSet = await removeSetFromLawnTennisGame(req, res);
             
        res.json({
            deletedSet,
            lawnTennisGameAfterDeletingSet
        });
    }
    catch (err) {
        res.status(500).json({
...
exports.setWinnerOfSet = async  (req, res) => {
    ...
    try{
        ...
        if(teamASetScore > teamBSetScore){
            ...
            const lawnTennisGameAfterUpdatedScores= await updateTeamScores(req, res);


            res.json({afterSettingWinnerOfSet, lawnTennisGameAfterUpdatedScores})
        }
