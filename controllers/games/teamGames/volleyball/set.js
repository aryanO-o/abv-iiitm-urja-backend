const Set = require("../../../../models/games/teamGames/volleyball/Set")
const { default: mongoose } = require("mongoose");
const { addSetToVolleyballGame, removeSetFromVolleyballGame, updateTeamScores } = require("./volleyball");
...
exports.addSet = async (req, res) => {
    ...
    try{
        ...
        const addedSetToVolleyballGame = await addSetToVolleyballGame(req, res);

        res.json({
            savedSet,
            addedSetToVolleyballGame
        })
    }
    catch(err) {
        res.status(500).json({
...
exports.removeSet = async (req, res) => {
    ...
    try {
        ...
        const volleyballGameAfterDeletingSet = await removeSetFromVolleyballGame(req, res);
             
        res.json({
            deletedSet,
            volleyballGameAfterDeletingSet
        });
    }
    catch (err) {
        res.status(500).json({
...
exports.setWinnerOfSet = async  (req, res) => {
    ...
    try{
        ...
        if(teamASetScore > teamBSetScore){
            ...
            const volleyballGameAfterUpdatedScores= await updateTeamScores(req, res);


            res.json({afterSettingWinnerOfSet, volleyballGameAfterUpdatedScores})
        }
